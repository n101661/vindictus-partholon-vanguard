...
`character slots` to join and `grand discovery points`.
...
- Gain 10 points for first character joined
- Gain 5 points for second and third character joined
- Gain 3 extra points per fit specialty of character joined
...
- The difficulty of mission affect gaining points of first character joined. For example, the mission of difficulty `2` allows two characters to join. If all characters are not fitting any specialty, the total points is 17(first character joined + difficulty + second character joined = 10 + 2 + 5).
- Each mission limits the maximum number of character to join

Now, you have `characters` characters to join missions,
each character has 5 specialties.

Given `missions` and `characters`, return joined indices
of character for missions to maximize the total
...
Output: [[0, 2], [1]]
Explanation: First mission gains 21 points(first character joined + second character joined + fit specialties = 10+5+2*3) while second mission gains 16 points(first character joined + fit specialties = 10+2*3). The total of probability is 21/30 + 16/40 = 1.1.
```
...
Output: [[0, 1], []]
Explanation: First mission gains 22 points(first character joined + mission difficulty + second character joined + fit specialties = 10+(-2)+5+3*3) while second mission gains 0 points. The total of probability is 22/25 + 0/100 = 0.88.
```
...
Output: [[0, 1, 2]]
Explanation: The mission gains 31 points(first character joined + mission difficulty + second character joined + third character joined + fit specialties = 10+2+5+5+3*3), but the limitation of points is 30, the actual points are 30. The total of probability is 30/30 = 1.
```
